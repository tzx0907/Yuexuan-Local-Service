<template>
  <div class="dashboard-container">
    <div class="container">
      <div class="tableBar">
        <label style="margin-right: 10px">商品/服务名称：</label>
        ...
        <label style="margin-right: 10px; margin-left: 20px">商品服务分类：</label>
...
export default class extends Vue {
  ...
  private deleteHandle(type: string, id: any) {
    ...
    this.$confirm('确认删除该商品/服务，是否继续？', '确认删除', {
  ...
  private statusHandle(row: any) {
    ...
    if (typeof row === 'string') {
      if (this.checkList.length === 0) {
        this.$message.error('批量操作，请先勾选商品或服务！')
        return false
      }
    ...
    this.$confirm('确认更改该商品/服务状态？', '提示', {
      ...
      dishStatusByStatus(this.dishState)
        .then(res => {
          if (res && res.data && res.data.code === 1) {
            this.$message.success('商品/服务状态已更新！')
            this.init()
          } else {
            this.$message.error(res.data.msg)
          }
