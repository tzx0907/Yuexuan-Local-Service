<template>
  <div class="navbar">
    <div class="statusBox">
      ...
      <span v-if="status===1"
            class="businessBtn">服务中</span>
      <span v-else
            class="businessBtn closing">暂停接单</span>
    </div>
    ...
    <div :key="restKey"
      ...
      <div class="rightStatus">
        ...
        <span class="navicon operatingState" @click="handleStatus"><i />服务状态设置</span>
      </div>
    ...
    <el-dialog title="服务状态设置"
      ...
      <el-radio-group v-model="setStatus">
        <el-radio :label="1">
          服务中
          <span>当前平台正在接收新订单并安排配送或服务履约。</span>
        </el-radio>
        <el-radio :label="0">
          暂停接单
          <span>当前平台暂停接收新订单，恢复服务中后即可正常接单。</span>
        </el-radio>
      </el-radio-group>
      <span slot="footer"
            class="dialog-footer">
...
export default class extends Vue {
  ...
  webSocket() {
    ...
    // 本地静态管理端未配置 WebSocket 时不建立空连接，也不向管理员弹出无关错误。
    ...
    if (typeof WebSocket == 'undefined') {
      ...
      this.websocket.onmessage = function (msg) {
        ...
        that.$notify({
          title: jsonMsg.type === 1 ? '待处理订单' : '订单提醒',
          duration: 0,
      ...
      this.websocket.onerror = function () {
        that.$notify({
          title: '实时提醒暂不可用',
          message: '订单管理功能不受影响；请确认后端 WebSocket 服务已配置后再启用提醒。',
          type: 'error',
          duration: 0,
        })
      }
