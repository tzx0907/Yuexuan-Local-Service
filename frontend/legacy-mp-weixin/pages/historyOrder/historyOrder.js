    ...
    var l0 =
      _vm.recentOrdersList && _vm.recentOrdersList.length > 0
        ? _vm.__map(_vm.recentOrdersList, function(item, index) {
            ...
            var m4 = (item.orderDetailList || []).some(function(detail) {
              return Number(detail.dishId) === 72 || /上门.*(服务|清洗)|服务.*上门/.test(String(detail.name || ''))
            })
            var m5 = item.estimatedDeliveryTime || ''
            return {
              $orig: $orig,
              m0: m0,
              m1: m1,
