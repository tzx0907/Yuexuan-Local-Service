...
var render = function() {
  ...
  // Do not render a nullable store-derived total.  The cart panel uses a
  // page-local array while the store refreshes asynchronously, so sum the
  // currently visible cart rows and always provide a currency string.
  var visibleCart = Array.isArray(_vm.cartItems) && _vm.cartItems.length > 0 ? _vm.cartItems : m0
  var cartTotal = visibleCart.reduce(function(sum, item) {
    return sum + Number(item && item.amount || 0) * Number(item && item.number || 0)
  }, 0)
  var g1 = cartTotal.toFixed(2)
  // WXML cannot reliably perform arithmetic through the Vue data bridge.
  // Supply fully formatted cart values to make the displayed sum auditable.
  var lCart = _vm.__map(visibleCart, function(item) {
    var $orig = _vm.__get_orig(item)
    var amount = Number(item.amount || 0)
    var number = Number(item.number || 0)
    return { $orig: $orig, unitPrice: amount.toFixed(2), lineTotal: (amount * number).toFixed(2) }
  })
