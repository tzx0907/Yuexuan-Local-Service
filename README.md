...
- 支付成功处理具有幂等性：同一订单的重复支付请求或重复支付回调只会首次更新订单、清空购物车并发送来单提醒，后续通知会安全返回，不重复执行副作用。
- 下单接口要求请求头 `Idempotency-Key`。Redis 通过 `SETNX` 快速拦截短时间内的重复请求；订单表通过 `(user_id, submit_request_id)` 联合唯一索引提供最终兜底。下单成功后，重复请求会返回第一次创建的订单结果。
